...
import (
	...
	"github.com/walteh/retab/gen/gopls/vulncheck/govulncheck"
...
func suggestGovulncheckAction(fromGovulncheck bool, uri span.URI) (source.SuggestedFix, error) {
	if fromGovulncheck {
		resetVulncheck, err := command.NewNoopCommand("Reset govulncheck result", command.ResetGoModDiagnosticsArgs{
	...
	vulncheck, err := command.NewNoopCommand("Run govulncheck to verify", command.VulncheckArgs{
...
func getUpgradeCodeAction(fh source.FileHandle, req *modfile.Require, version string) (protocol.Command, error) {
	cmd, err := command.NewNoopCommand(upgradeTitle(version), command.DependencyArgs{
		URI:        protocol.URIFromSpanURI(fh.URI()),
		AddRequire: false,
