...
func NewOrderedMapFromKVPairs(pairs MapSlice) (*OrderedMap, error) {
	om := NewOrderedMap()
	for _, pair := range pairs {
		if key, ok := pair.Key.(string); ok {
			om.Set(key, pair.Value)
		} else {
			return nil, fmt.Errorf("key %v is not a string", pair.Key)
		}
	}
	return om, nil
}

func (om *OrderedMap) ToMapSlice() MapSlice {
	var result MapSlice
	for key, value := range om.m {
		result = append(result, MapItem{key, value})
	}
	return result
}
...
func (om *OrderedMap) EntriesIter() func() (*MapItem, bool) {
	e := om.l.Front()
	return func() (*MapItem, bool) {
		if e != nil {
			key := e.Value.(string)
			e = e.Next()
			return &MapItem{key, om.m[key]}, true
		}
		return nil, false
	}
}
...
func (om *OrderedMap) EntriesReverseIter() func() (*MapItem, bool) {
	e := om.l.Back()
	return func() (*MapItem, bool) {
		if e != nil {
			key := e.Value.(string)
