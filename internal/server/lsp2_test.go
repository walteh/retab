...
import (
	...
	"github.com/walteh/retab/gen/gopls/tests"
...
func (r *runner) AddImport(t *testing.T, uri span.URI, expectedImport string) {
	cmd, err := command.NewNoopCommand("List Known Packages", command.URIArg{
		URI: protocol.URIFromSpanURI(uri),
	})
	if err != nil {
		t.Fatal(err)
	...
	if !hasPkg {
		t.Fatalf("%s: got %v packages\nwant contains %q", command.Noop, res.Packages, expectedImport)
	}
	cmd, err = command.NewNoopCommand("Add Imports", command.AddImportArgs{
		URI:        protocol.URIFromSpanURI(uri),
