...
// 	"github.com/walteh/retab/gen/gopls/filecache"
...
// func (snapshot *snapshot) Analyze(ctx context.Context, pkg struct{}, analyzers []*source.Analyzer, reporter *progress.Tracker) ([]*source.Diagnostic, error) {
// 	start := time.Now() // for progress reporting

// 	ctx, done := event.Start(ctx, "snapshot.Analyze", tag.Package.Of(snapshot.view.name))
// 	defer done()
...
// 	var makeNode func(from *analysisNode) (*analysisNode, error)
// 	makeNode = func(from *analysisNode) (*analysisNode, error) {
// 		if node == nil {
...
// 			node.files = make([]source.FileHandle, len(snapshot.overlays()))
// 			for i, uri := range snapshot.overlays() {
// 				fh, err := snapshot.ReadFile(ctx, uri.uri)
// 				if err != nil {
// 					return nil, err
// 				}
// 				node.files[i] = fh
...
// 			node.preds = append(node.preds, from)
// 		}
// 		atomic.AddInt32(&node.unfinishedPreds, 1)
// 		return node, nil
// 	}

// 	// For root packages, we run the enabled set of analyzers.
// 	roots := []*analysisNode{}
// 	root, err := makeNode(nil)
// 	if err != nil {
// 		return nil, err
// 	}
...
// 					completed, 1)
// 				pct := 100 * float64(completed) / float64(1)
...
// 	succs           *analysisNode        //   (preds -> self -> succs)
// 	unfinishedSuccs int32
// 	unfinishedPreds int32           // effectively a summary.Actions refcount
// 	allDeps         *analysisNode   // all dependencies including self
// 	exportDeps      *analysisNode   // subset of allDeps ref'd by export data (+self)
...
// 	vdeps *analysisNode // vertical dependencies
