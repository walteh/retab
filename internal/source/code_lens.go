...
func runTestCodeLens(ctx context.Context, snapshot Snapshot, fh FileHandle) ([]protocol.CodeLens, error) {
	...
	for _, fn := range fns.Tests {
		cmd, err := command.NewNoopThreeArgsCommand("run test", puri, []string{fn.Name}, nil)
		if err != nil {
			return nil, err
		}
	...
	for _, fn := range fns.Benchmarks {
		cmd, err := command.NewNoopThreeArgsCommand("run benchmark", puri, nil, []string{fn.Name})
		if err != nil {
			return nil, err
		}
		rng := protocol.Range{Start: fn.Rng.Start, End: fn.Rng.Start}
	...
	if len(fns.Benchmarks) > 0 {
		...
		cmd, err := command.NewNoopThreeArgsCommand("run file benchmarks", puri, nil, benches)
		if err != nil {
			return nil, err
		}
		codeLens = append(codeLens, protocol.CodeLens{Range: rng, Command: &cmd})
...
func goGenerateCodeLens(ctx context.Context, snapshot Snapshot, fh FileHandle) ([]protocol.CodeLens, error) {
	...
	for _, c := range pgf.File.Comments {
		for _, l := range c.List {
			...
			nonRecursiveCmd, err := command.NewNoopCommand("run go generate", command.GenerateArgs{Dir: dir, Recursive: false})
			if err != nil {
				return nil, err
			}
			recursiveCmd, err := command.NewNoopCommand("run go generate ./...", command.GenerateArgs{Dir: dir, Recursive: true})
			if err != nil {
				return nil, err
			}
...
func regenerateCgoLens(ctx context.Context, snapshot Snapshot, fh FileHandle) ([]protocol.CodeLens, error) {
	...
	puri := protocol.URIFromSpanURI(fh.URI())
	cmd, err := command.NewNoopCommand("regenerate cgo definitions", command.URIArg{URI: puri})
	if err != nil {
		return nil, err
	}
...
func toggleDetailsCodeLens(ctx context.Context, snapshot Snapshot, fh FileHandle) ([]protocol.CodeLens, error) {
	...
	puri := protocol.URIFromSpanURI(fh.URI())
	cmd, err := command.NewNoopCommand("Toggle gc annotation details", puri)
	if err != nil {
		return nil, err
	}
	return []protocol.CodeLens{{Range: rng, Command: &cmd}}, nil
